...
export const STRINGS = {
  ...
  'conc.std':         { zh: '标准（≤80 并发，默认）', en: 'Standard (≤80 concurrent, default)' },
  ...
  'adv.distributed':  { zh: '底层为分布式存储（vSAN 等），冗余由存储层提供', en: 'Distributed storage underneath (vSAN etc.), redundancy at the storage layer' },
  'adv.distributed.hint': { zh: '勾选后不再部署 mirror，可用容量近乎翻倍；数据冗余完全依赖存储层。注意：当前开源 Cloudberry 尚不支持 mirrorless 部署，需联系 Cloudberry 商业支持厂商适配，或自行适配相关平台能力', en: 'Skips segment mirrors — usable capacity nearly doubles; redundancy relies entirely on the storage layer. Note: open-source Cloudberry does not support mirrorless deployment out of the box — engage a commercial Cloudberry support vendor or adapt the platform capability yourself' },
  ...
  'note.datanode.nomirror': { zh: '无 mirror：冗余由存储层多副本提供（云盘/分布式存储，支持快照）。mirrorless 需商业支持厂商适配，开源版暂不支持', en: 'No mirrors: redundancy from the replicated storage layer (managed disks / distributed storage, snapshot-capable). Mirrorless needs commercial-vendor adaptation; not supported by open-source Cloudberry out of the box' },
  'note.azure.extra':      { zh: '无 mirror（Premium SSD 多副本，需商业支持厂商适配）；建议 UDP interconnect，预留端口 65330', en: 'No mirrors (replicated Premium SSD; needs commercial-vendor adaptation); use UDP interconnect, reserve port 65330' },
  'note.gcp.extra':        { zh: '无 mirror（pd-ssd 多副本，需商业支持厂商适配）；GCP 上建议用更多小节点（相对 AWS）', en: 'No mirrors (replicated pd-ssd; needs commercial-vendor adaptation); prefer more, smaller nodes on GCP (vs AWS)' },
