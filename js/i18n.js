...
export const STRINGS = {
  ...
  'adv.distributed.hint': { zh: '勾选后不再部署 mirror，可用容量近乎翻倍；数据冗余完全依赖存储层', en: 'Skips segment mirrors — usable capacity nearly doubles; data redundancy relies entirely on the storage layer' },
  ...
  'note.datanode.nomirror': { zh: '无 mirror：冗余由存储层多副本提供（云盘/分布式存储，支持快照）', en: 'No mirrors: redundancy from the replicated storage layer (managed disks / distributed storage, snapshot-capable)' },
  'note.azure.extra':      { zh: '无 mirror（Premium SSD 多副本）；建议 UDP interconnect，预留端口 65330', en: 'No mirrors (replicated Premium SSD); use UDP interconnect, reserve port 65330' },
  'note.gcp.extra':        { zh: '无 mirror（pd-ssd 多副本）；GCP 上建议用更多小节点（相对 AWS）', en: 'No mirrors (replicated pd-ssd); prefer more, smaller nodes on GCP (vs AWS)' },
